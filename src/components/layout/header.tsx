    ...
    <header>
      <nav
        ...
        className={cn(
          ...
          scrolled &&
            "border-border/50 bg-background/80 backdrop-blur-2xl shadow-sm"
        )}
      >
                ...
                <ul className="flex gap-1 text-sm">
                  {menuItems.map((item, index) => (
                    <li key={index}>
                      <Link
                        href={item.href}
                        className="text-muted-foreground hover:text-foreground block rounded-lg px-3 py-1.5 transition-all duration-200 hover:bg-muted"
                      >
                        <span>{item.name}</span>
                      </Link>
                    </li>
